...
execute in data:map as @a[scores={EquipmentSlot1=3..4},tag=!EqS1used] if entity @s[nbt={SelectedItemSlot:2}] at @s anchored eyes run summon armor_stand ^ ^ ^1 {HandItems:[{id:"green_shulker_box",Count:1}],NoGravity:1b,Small:1b,NoBasePlate:1b,Tags:[Grenade]}
execute in data:map as @a[scores={EquipmentSlot1=3..4},tag=!EqS1used] if entity @s[nbt={SelectedItemSlot:2}] at @s anchored eyes positioned ^ ^ ^1 run scoreboard players set @e[limit=1,sort=nearest,tag=Grenade] Motion 50
