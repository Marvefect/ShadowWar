...
execute as @e[type=interaction,tag=Station] at @s in data:map if score @p[distance=..3,tag=Hacking,team=Red] Time matches 99.. as @a at @s run playsound minecraft:item.shield.break player @a
...
execute as @e[type=interaction,tag=Station] at @s in data:map if score Round GameStatus matches 1..2 if score @p[distance=..3,tag=Hacking,team=Blue] Time matches 99.. as @a at @s run playsound minecraft:item.shield.block player @a
