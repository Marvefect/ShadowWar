...
#execute as @a[scores={Jump=2..},predicate=data:jump] unless score @s ArmorType matches 0..1 at @s unless block ~ ~-0.25 ~ air run tp @s ~ ~ ~ facing ^ ^ ^-1
#execute as @a[scores={Jump=2..}] at @s unless block ~ ~-0.2 ~ air run scoreboard players reset @s Jump
execute as @a[scores={ArmorType=3..}] run effect give @s jump_boost 1 128 true
